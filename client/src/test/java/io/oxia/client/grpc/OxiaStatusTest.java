...
class OxiaStatusTest {
    ...
    /** Simulates a gRPC error with the custom Oxia code in grpc-status-details-bin trailer. */
    private static StatusRuntimeException oxiaErrorWithTrailer(
            OxiaStatus oxiaStatus, String message) {
        var rpcStatus =
    ...
    /** Simulates a plain gRPC error without trailer (how Go server sends simple errors). */
    private static StatusRuntimeException oxiaErrorWithoutTrailer(String message) {
        return Status.UNKNOWN.withDescription(message).asRuntimeException();
    }
    ...
    class FromError {
        @Test
        void extractsFromTrailer() {
            var err =
                    oxiaErrorWithTrailer(OxiaStatus.NODE_IS_NOT_LEADER, "node is not leader for shard 0");
            assertThat(OxiaStatus.fromError(err)).isEqualTo(OxiaStatus.NODE_IS_NOT_LEADER);
        }

        @Test
        void fallsBackToDescriptionWhenNoTrailer() {
            var err = oxiaErrorWithoutTrailer("oxia: namespace not found");
    ...
    class IsRetriableTests {
        ...
        void invalidStatusIsRetriable() {
            var err = oxiaErrorWithTrailer(OxiaStatus.INVALID_STATUS, "oxia: invalid status");
            assertThat(OxiaStatus.isRetriable(err)).isTrue();
        }

        @Test
        void alreadyClosedIsRetriable() {
            var err = oxiaErrorWithTrailer(OxiaStatus.ALREADY_CLOSED, "oxia: resource is already closed");
            assertThat(OxiaStatus.isRetriable(err)).isTrue();
        }

        @Test
        void sessionNotFoundIsNotRetriable() {
            var err = oxiaErrorWithTrailer(OxiaStatus.SESSION_NOT_FOUND, "oxia: session not found");
            assertThat(OxiaStatus.isRetriable(err)).isFalse();
        }

        @Test
        void namespaceNotFoundIsNotRetriable() {
            var err = oxiaErrorWithTrailer(OxiaStatus.NAMESPACE_NOT_FOUND, "oxia: namespace not found");
            assertThat(OxiaStatus.isRetriable(err)).isFalse();
        }
    }
}
